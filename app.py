...
# 2. LISTA DE ALVOS (Para o Top 5 ser rápido)
LISTA_ALVOS = [
...
def buscar_ranking_automatico():
    ...
    # Lógica Inteligente: Se for antes do dia 10, pega o mês anterior
    if agora.day < 10:
        if agora.month == 1:
...
if not df_all.empty:
    ...
    # 6. TABELA DE RESULTADOS (AGORA COM LINKS E R$)
    if id_final:
        ...
        try:
            ...
            if dados_finais:
                ...
                df_f.columns = ['Data', 'Tipo', 'Fornecedor', 'Valor', 'Nota Fiscal']
                
                # CONFIGURAÇÃO AVANÇADA DA TABELA
                st.dataframe(
                    df_f,
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "Nota Fiscal": st.column_config.LinkColumn(
                            "Nota Fiscal", 
                            display_text="📄 Ver Nota",
                            help="Clique para abrir o documento oficial"
                        ),
...
with col_pix_img:
    if os.path.exists("pix.jpeg"):
        ...
        st.info("📷 QR Code (Adicione pix.jpeg na pasta)")
